...
```bash
conda create python=3.8 -n schoolmenu
```
2 - <a name="step-2">Activate the conda environment</a>

```bash
conda activate schoolmenu
```
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///schoolmenu` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE schoolmenu WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE schoolmenu TO ;
```
...
ERROR:  database "schoolmenu" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists schoolmenu
